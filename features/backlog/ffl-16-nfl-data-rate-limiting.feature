...
Feature: NFL Data Polling Rate Limiting
  As a system
  I want to implement rate limiting for nflverse data polling
  So that I respect data update schedules and avoid unnecessary fetches

  Background:
    Given the system integrates with nflreadpy (nflverse) for NFL data
    And nflverse data updates every 5 minutes for schedules and 1 minute for live games
    And the system implements rate limiting for polling frequency
