...
@DisplayName("PauseLeagueUseCase Tests")
class PauseLeagueUseCaseTest {

    @Mock
    private LeagueRepository leagueRepository;

    private PauseLeagueUseCase useCase;
    ...
    @BeforeEach
    void setUp() {
        useCase = new PauseLeagueUseCase(leagueRepository);
    ...
    class Execute {
        ...
        @Test
        @DisplayName("should pause active league successfully")
        void shouldPauseActiveLeagueSuccessfully() {
            // Arrange
            PauseLeagueUseCase.PauseLeagueCommand command =
            ...
            assertEquals(LeagueStatus.PAUSED, result.getStatus());
            assertNotNull(result.getPausedAt());

            verify(leagueRepository).save(leagueCaptor.capture());
            assertEquals(LeagueStatus.PAUSED, leagueCaptor.getValue().getStatus());
        }
        ...
        void shouldThrowLeagueNotFoundException() {
            ...
            PauseLeagueUseCase.PauseLeagueCommand command =
                    new PauseLeagueUseCase.PauseLeagueCommand(unknownLeagueId);

            when(leagueRepository.findById(unknownLeagueId)).thenReturn(Optional.empty());

            // Act & Assert
            PauseLeagueUseCase.LeagueNotFoundException exception = assertThrows(
                    PauseLeagueUseCase.LeagueNotFoundException.class,
        ...
        @Test
        @DisplayName("should set pausedAt timestamp when pausing")
        void shouldSetPausedAtTimestamp() {
            // Arrange
            PauseLeagueUseCase.PauseLeagueCommand command =
                    new PauseLeagueUseCase.PauseLeagueCommand(leagueId);
        ...
        @Test
        @DisplayName("should save league after pausing")
        void shouldSaveLeagueAfterPausing() {
            // Arrange
            PauseLeagueUseCase.PauseLeagueCommand command =
                    new PauseLeagueUseCase.PauseLeagueCommand(leagueId);
